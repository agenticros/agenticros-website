...
const packages = [
  { name: '@agenticros/core', desc: 'Shared transport layer, Zod config schema, mission runner, capability registry, and memory backends — no platform dependencies' },
  { name: '@agenticros/ros-camera', desc: 'Shared camera snapshot encoding for sensor_msgs/Image and CompressedImage' },
  { name: 'agenticros', desc: <>The unified <strong>CLI</strong>. <code className="rounded bg-bg-elevated px-1.5 py-0.5 font-mono text-sm text-coral-bright">npx agenticros</code> on any Node &ge; 20 host: interactive menu, real-robot launcher, simulator launcher, fleet management, doctor, log tail, config editor. Bundles a snapshot of the workspace so it works without a checkout.</> },
